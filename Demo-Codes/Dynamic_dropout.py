...
def load_data(path="final_dataset_multidistrict_historical.csv",exclude_district=49,):
    ...
    return district_index,dtrain_list,dtrain_label_list,deval_list,dtest_list

#%% 只要残留的地区数量大于1，就可以进行federated learning（包含绘图第1部分）
district_index,dtrain_list,dtrain_label_list,deval_list,dtest_list=load_data()
...
while len(district_index)>=1:
    train_data=pd.concat(dtrain_list)
    train_label=pd.concat(dtrain_label_list)
    dtrain=xgb.DMatrix(train_data,label=train_label)
    eval_dict={}
    model.train(dtrain,max_depth=MAX_DEPTH,num_boost_round=500,evals=deval_list,evals_result=eval_dict,eta=0.2,verbose_eval=0)
    eval_dict={int(key):eval_dict[key]['NLL'] for key in eval_dict}
    eval_dict_list.append(eval_dict)
    drop_IDX=[]
    pointer=0
    while pointer<len(district_index):
    # 寻找到底哪些参与方过拟合了，并且从district_index中删除这些参与方（进入transfer阶段）
        IDX=district_index[pointer]
        list_NLL=eval_dict[IDX]
        if len(list_NLL)<2 or list_NLL[-2]<list_NLL[-1]:
            if dtest_list[pointer].num_row():
                df_prediction_dict[IDX]=model.predict(dtest_list[pointer],normalized=True)
            IDX=district_index[pointer]
            ...
            drop_IDX.append(IDX)
            eval_dict_transfer_temp={}         
            model.train_transfer(xgb.DMatrix(dtrain_list[pointer],label=dtrain_label_list[pointer]),IDX,max_depth=MAX_DEPTH,num_boost_round=500,evals=[deval_list[pointer]],
                                    evals_result=eval_dict_transfer_temp,eta=0.2,verbose_eval=0)
            eval_transfer_dict[IDX]=eval_dict_transfer_temp[str(IDX)]['NLL']
            if dtest_list[pointer].num_row():
                df_prediction_transfer_dict[IDX]=model.predict_transfer(dtest_list[pointer],IDX,normalized=True)
            if IDX==district_of_interest:
            ...
            district_index.pop(pointer)
            dtrain_list.pop(pointer)
            dtrain_label_list.pop(pointer)
            deval_list.pop(pointer)
            dtest_list.pop(pointer)
        else:
            pointer+=1
    assert len(drop_IDX)>=1,"警告：没有参与方退出！"
    drop_index_list.append(drop_IDX)
...
for i in range((model.xgb_model.trees_to_dataframe()['Tree'].max()+1)//2):
    df_11=model.predict(deval_11,ntree_limit=i+1,normalized=True)
    final_list.append(-np.nansum(norm.logpdf(x=df_11['label'],loc=df_11['mu'],scale=df_11['sigma']))/df_11.shape[0])
bst_epoch=int(np.argmin(final_list))

df_bst_test=model.predict(dtest_11,ntree_limit=bst_epoch+1,normalized=True)
mu_bst_test=np.array(df_bst_test['mu'])[:,None]
sigma_bst_test=Gaussian.Softplus_inv(np.array(df_bst_test['sigma']))[:,None]

df_bst_valid=model.predict(deval_11,ntree_limit=bst_epoch+1,normalized=True)
mu_bst_valid=np.array(df_bst_valid['mu'])[:,None]
sigma_bst_valid=Gaussian.Softplus_inv(np.array(df_bst_valid['sigma']))[:,None]

df_bst_train=model.predict(dtrain_11,ntree_limit=bst_epoch+1,normalized=True)
...
MAX_DEPTH=3  # !!!MAX_DEPTH=3或4都可以或多或少实现效果，请注意改动
eval_dict={}
model2.train(dtrain_11,max_depth=MAX_DEPTH,num_boost_round=500,evals=[(deval_11,"49_test")],evals_result=eval_dict,eta=0.2,verbose_eval=0,reset_base_margin=False)
...
#%% 将loss进行导出
import pickle
directory='Training-Pickle/'
# 导出eval_dict_list和eval_transfer_dict
with open(directory+'eval_dict_list.pickle', 'wb') as f:
    pickle.dump(eval_dict_list, f)
...
with open(directory+'res_temp_dynamic.pickle', 'wb') as f:
    pickle.dump(res_temp, f)
with open(directory+'res_temp_transfer_dynamic.pickle', 'wb') as f:
